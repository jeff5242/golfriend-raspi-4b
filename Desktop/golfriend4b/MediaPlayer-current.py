...
class MediaPlayer(QtWidgets.QWidget):
    ...
    def __init__(self, app, parent):
        ...
        self.clockLabel.setStyleSheet("color: black; font-size: 32px; background-color: rgba(255,255,255,0.6);")
        self.clockLabel.move(30, 30)
        self.clockLabel.resize(800, 50)
        self.clockLabel.setText("00:00:00 | IP")
        self.clockLabel.show()

        self.elapsedSeconds = 0
        self.clockTimer = QtCore.QTimer()
        self.clockTimer.timeout.connect(self.updateClockLabel)
        ...
        try:
            ...
            print('IP:{}'.format(self.device_ip))
            print('Device Name:{}'.format(self.DeviceName))
        except:
            self.device_ip = "Unavailable"
            print('⚠️ 無法取得網路資訊')
    ...
    def updateClockLabel(self):
        ...
        time_str = self.getClockString(self.elapsedSeconds)
        ip = getattr(self, "device_ip", "IP?")
        name = getattr(self, "DeviceName", "Device?")
        self.clockLabel.setText(f"{time_str} | {ip} | {name}")
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key_Space:
            MediaStatus = not MediaStatus
            print("play" if MediaStatus else "pause")
            if MediaStatus:
                self.showStandbyImage()
            ...
            self.showStandbyImage()
            MediaStatus = True
        elif event.key() in [Qt.Key_Tab, Qt.Key_Percent, 16777248] or event.text() == '%':
            print("MediaPlay, keyTab / % / Ctrl key pressed")
    ...
    def startQRCodeDailog(self):
        ...
        if httpR.error_code:
            if httpR.error_description != "NoWireless":
                ErrorDialog(self, httpR.error_description, httpR.transaction_code, self.Log).exec_()
        elif httpR.ProcessTag == "TakeBall":
            TakeBallConfirmDialog(self, httpR, self.Log, 2).exec_()
            if httpR.error_code:
                ErrorDialog(self, httpR.error_description, httpR.transaction_code, self.Log).exec_()
            elif httpR.ProcessTag == "TakeBallConfirm":
                SuccessDialog(self, httpR, self.Log, self.Control).exec_()
    ...
    def startDailog(self):
        ...
        if httpR.error_code:
            ...
            TakeBallConfirmDialog(self, httpR, self.Log).exec_()
            if httpR.error_code:
                ErrorDialog(self, httpR.error_description, httpR.transaction_code, self.Log).exec_()
            elif httpR.ProcessTag == "TakeBallConfirm":
                SuccessDialog(self, httpR, self.Log, self.Control).exec_()
