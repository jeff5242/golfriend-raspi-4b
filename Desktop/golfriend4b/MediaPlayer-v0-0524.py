...
##        httpR.SendPingCmd()
    ...
    def startQRCodeDailog(self):
        ...
        MediaStatus=False
        self.stopVideo("e")
        self.Log.i("startQRCodeDailog============startTrade")
        ...
        if(httpR.error_code):
            ...
            if(httpR.error_code):
                ...
                Successdialog.exec_()
        self.Log.i("startQRCodeDailog============endTrade")
        self.playFile()
        MediaStatus = True
    ...
    def playFile(self):
        ...
        if platform.system() == "Darwin":
        ...
        elif platform.system() == "Linux" and Path("/usr/bin/omxplayer").exists():
            print("▶️ Playing with omxplayer")
            self.process = subprocess.Popen(
                ['/usr/bin/omxplayer', '-o', settings.get('AudioMode', 'local'), self.file],
            ...
            print("▶️ Playing with QMediaPlayer")
            self.videoWidget = QVideoWidget(self)
            self.videoWidget.setGeometry(0, 0, self.playerWidth, self.playerHeight)
            self.videoWidget.show()

            self.qplayer = QMediaPlayer(None, QMediaPlayer.VideoSurface)
            self.qplayer.setMedia(QMediaContent(QUrl.fromLocalFile(self.file)))
            self.qplayer.setVideoOutput(self.videoWidget)
            self.qplayer.play()
            self.clock.start(1000)


        def play_with_best_available_player(filepath):
            # 音訊輸出參數 (從 settings 抓取)
            audio_mode = settings.get('AudioMode', 'local')  # 預設 local，如果沒定義
            ...
            if platform_name == "Darwin":
                ...
                subprocess.Popen(["mpv", "--fs", "--loop", filepath])
            elif shutil.which("cvlc"):
                print("🎬 Playing with cvlc")
                subprocess.Popen([
                ...
                print("❌ 沒有找到可用的播放器 (mpv / cvlc / open)")
                return False
            return True

        success = play_with_best_available_player(self.file)
        if not success:
            self.Log.e("MediaPlayer_playFile: 沒有可用播放器")
            return
        ...
        self.videoWidth = 0
        self.videoHeight = 0
        self.progressText.setText("00:00:00")
        self.clock.start(1000)
...
##        self.connectStatus.setFont(font)
        ...
        print("size.width(), size.height())=1820x1080")
...
class TakeBallConfirmDialog(QtWidgets.QDialog):
    def __init__(self, parent, httpR, tmplog, autoconfirm_second=0):
        ...
        self.Log = tmplog
        self.Log.i("TakeBallConfirmDialog_Initial,    "+"交易號碼"+ " :  " + httpR.response_id+", "+"取球數量" + " :  " + str(httpR.response_take)+", "+"剩餘球數" + " :  " +str(httpR.response_ball_before))
        ...
        # 自動取得螢幕解析度設定 Dialog 尺寸
        screen = QtWidgets.QApplication.primaryScreen()
        size = screen.size()
        self.resize(size.width(), size.height())
        print("size.width(), size.height()"+str(size.width())+","+str(size.height()))
        # self.resize(int(1920),int(1080))
        self.tmpHttpR = httpR
        self.Lock = False
        self.timeout = None
        # 如果傳入 autoconfirm 秒數大於 0，就啟動定時自動確認
        if autoconfirm_second > 0:
            QtCore.QTimer.singleShot(autoconfirm_second * 1000, self.autoConfirm)
        else:
            self.start_timer(10)  # 原本的 timeout
...
class ErrorDialog(QtWidgets.QDialog):
    def __init__(self, parent, ErrorTitle, ErrorMsg, tmplog):
        ...
        self.Log = tmplog
        self.Log.e("ErrorDialog_Initial,    Title:"+ErrorTitle+", Msg:"+ErrorMsg)
...
class SuccessDialog(QtWidgets.QDialog):
    def __init__(self, parent, httpR, tmplog, tmpBallControl):
        ...
        self.Log = tmplog
        self.Log.i("SuccessDialog_Initial,    "+"交易號碼"+ " :  " + httpR.response_id+", "+"剩餘球數" + " :  " +str(httpR.response_ball_after))
