...
#20180307 End
	...
	def SendTakeBallCmd(self):
		...
		sha1 =  hashlib.sha1("salt" + currentTime + self.transaction_code + self.device_code).hexdigest()
		payload = {
		...
		r = requests.post("http://golfpoint.milkidea.com/api/take_ball", payload);
	...
	def SendTakeBallConfirmCmd(self, note, status):
		...
		currentTime = str(int(time.mktime(datetime.datetime.now().timetuple())))
		sha1 =  hashlib.sha1("salt" + currentTime + self.response_id).hexdigest()
		payload = {
		...
		r = requests.post("http://golfpoint.milkidea.com/api/take_ball_confirm", payload);
		print(r.status_code, r.reason)
		print(r.text)
		if(r.status_code==200 or r.status_code == 400):
	...
	def SendGetSettingCmd(self):
		...
		currentTime = str(int(time.mktime(datetime.datetime.now().timetuple())))
		sha1 =  hashlib.sha1("salt" + currentTime + str(self.response_timer) + self.device_code).hexdigest()
		payload = {
			'device_code':self.device_code,
			'config':self.config,
			'time':currentTime,
		...
		r = requests.post("http://golfpoint.milkidea.com/api/take_ball_set", payload);
		print(r.status_code, r.reason)
		print(r.text)
	...
	def SendPingCmd(self, note):
		...
		currentTime = str(int(time.mktime(datetime.datetime.now().timetuple())))
		sha1 =  hashlib.sha1("salt" + currentTime + note + self.device_code).hexdigest()
		payload = {
			'device_code':self.device_code,
			'time':currentTime,
			'note':note ,
		...
		r = requests.post("http://golfpoint.milkidea.com/api/take_ball_ping", payload);
		print(r.status_code, r.reason)
		print(r.text)
